...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 项目调度 <span class="c-gray en">&gt;</span> 我的调度任务 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div >
        <form action="<%=request.getContextPath()%>/project/searchproject" method="post">
            ...
            任务期号：<input type="text" name="type"   style="width:170px" class="input-text">
    ...
    <form id="myform" name="myform" method="post" action="<%=request.getContextPath()%>/project/projectcheck">
            ...
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer">
                    ...
                    <tr class="text-c">
                        ...
                        <th width="50">填报情况</th>
                        <th width="50">任务名称</th>
                        <th width="70">日常监管责任单位</th>
                        <th width="100">日常监管责任单位审核意见</th>
                        <th width="60">建设地点</th>
                        <th width="60">总投资</th>
                        <th width="60">本次下达资金</th>
                        <th width="60">任务类型</th>
                    ...
                    <c:forEach items="${pageInfo.list}" var="i" varStatus="suoyin1" >
                        ...
                        <tr class="text-c">
                            ...
                            <td><c:forEach items="${sessionScope.rollplans}" var="pt" varStatus="suoyin2">
                                <c:if test="${pt.projectid ==i.projectid }"><a href="<%=request.getContextPath()%>/project/tiaoduwrite?schedulingid=${i.schedulingid}&projectid=${i.projectid }&rollplanid=${pt.rollplanid}">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/project/tiaoducountyguanli?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/project/tiaoducountyguanli?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/project/tiaoducountyguanli?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/project/tiaoducountyguanli?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
