...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
           ...
           <input type="button" class="btn btn-primary radius" id="button" value="报送"  onclick="projectsend()"  />
    ...
    <div class="mt-20">
        <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
            ...
            <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/project/searchproject?pn=1&name='<%=name%>'&type='<%=type%>'&property='<%=property%>'">首页</a>
                <c:if test="${searchproject.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/project/searchproject?pn=${searchproject.pageNum-1}&name='<%=name%>'&type='<%=type%>'&property='<%=property%>'"
                ...
                <c:forEach items="${searchproject.navigatepageNums }" var="page_Num">
                    ...
                    <c:if test="${page_Num != searchproject.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/project/searchproject?pn=${ page_Num}&name='<%=name%>'&type='<%=type%>'&property='<%=property%>'">${
                                page_Num}</a>
                        </span>
                    </c:if>
                </c:forEach>
                ...
                <a class="paginate_button next disabled"
                   href="<%=request.getContextPath()%>/project/searchproject?pn=${searchproject.pages}&name='<%=name%>'&type='<%=type%>'&property='<%=property%>'">末页</a>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function projectsend( ) {
            ...
            }else if($("input[type='checkbox']:checked").length>1)
            {
                ...
                if (window.confirm("确定报送项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/project/projectcheck?ids=" + result;
                }
            }
        }
        ...
        function dealmore( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定删除这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/project/dealmore?ids=" + result;
                }
            }
        }
    </script>
