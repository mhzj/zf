    ...
    <div class="mt-20">
        <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
            <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer">
                ...
                <td>${i.investment}</td>
                <td>${i.institution}</td>
                <td><c:forEach items="${sessionScope.projectsends}" var="pt" varStatus="suoyin2">
                    ...
                    <c:otherwise>
                       <c:forEach items="${sessionScope.projectsends}" var="pt" varStatus="suoyin2">
            ...
            <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/project/allproject?pn=1">首页</a>

                <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/project/allproject?pn=${pageInfo.pageNum-1}"
                ...
                <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                    ...
                    <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/project/allproject?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                    </c:if>
                </c:forEach>
                ...
                <a class="paginate_button next disabled"
                   href="<%=request.getContextPath()%>/project/allproject?pn=${pageInfo.pages}">末页</a>
            </div>
        </div>
    </div>
