        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/yearplan/citybaosong?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/yearplan/citybaosong?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/yearplan/citybaosong?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/yearplan/citybaosong?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
        ...
        function tuihui( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定退回这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/yearplan/citybaosongtuihui?ids=" + result;
                }
            }
        }
        <%--function baosong() {--%>
        ...
        function  naru() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("您确定将这" + count + "个项目纳入本级年度投资计划不报送吗?")) {
                    window.location.href = "<%=request.getContextPath()%>/yearplan/citybaosongnaru?ids=" + result;
                }
            }
        }
        function binageng() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定对这个项目进行变更文号?")) {
                    window.location.href = "<%=request.getContextPath()%>/yearplan/citybiangeng?yearplanid=" + result;
                }
            }
        }
    </script>
