...
<script type="text/javascript">
    ...
    if (window.confirm("你确定要入本级库同时进入本级滚动计划吗？")) {
        ...
        window.location.href = "<%=request.getContextPath()%>/review/citytwoguiku?ids=<%=ids%>&librarytype="+librarytype;
        }
    }
    function onlyone() {
        if (window.confirm("你确定要只入本级库吗？")) {
            var librarytype=document.getElementById('librarytype').value
            window.location.href = "<%=request.getContextPath()%>/review/cityoneguiku?ids=<%=ids%>&librarytype="+librarytype;
        }
    }
</script>
</body>
