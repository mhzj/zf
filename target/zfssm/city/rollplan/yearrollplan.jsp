...
<div class="page-container">
    ...
    <form id="myform" name="myform" method="post" action="">
        ...
        <span class="l">
            <input type="button" class="btn btn-primary radius"  value="退回待报区"  onclick="tuihui()"  />
        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/rollplan/yearrollplan?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/rollplan/yearrollplan?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/rollplan/yearrollplan?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/rollplan/yearrollplan?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
        ...
        function jiasuo() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("加锁后下级报送部门将无法对项目进行修改,您确定加锁这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/cityyearrollplanjiasuo?ids=" + result;
                }
            }
        }
        function  jiesuo() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("解锁后将允许下级报送部门对项目进行修改,您确定解锁这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/cityyearrollplanjiesuo?ids=" + result;
                }
            }
        ...
        function tuihui() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定退回这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/cityyearrollplantuihui?ids=" + result;
                }
            }
        }
    </script>
