...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 项目管理 <span class="c-gray en">&gt;</span> 已报区 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    ...
    <div class="mt-20">
        <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
            <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer">
                ...
                <c:forEach items="${pageInfo.list}" var="i" varStatus="suoyin1" >
                    ...
                    <tr class="text-c">
                        ...
                        <td>${i.rate}</td>
                        <td>${i.saverate}</td>
                        <td><c:forEach items="${sessionScope.projects}" var="pt" varStatus="suoyin2">
                            <c:if test="${pt.projectid==i.projectid}"><a href="<%=request.getContextPath()%>/review/projectvisit?projectid=${i.projectid}">${pt.name}</a></c:if>
                        </c:forEach></td>
                        <td><c:forEach items="${sessionScope.projects}" var="pt" varStatus="suoyin2">
                                <c:if test="${pt.projectid==i.projectid }">${pt.type}</c:if>
                            </c:forEach>
                        </td>
                        <td><c:forEach items="${sessionScope.projects}" var="pt" varStatus="suoyin2">
                        <c:if test="${pt.projectid ==i.projectid }">${pt.sendstatus}</c:if>
                        </c:forEach>
                        </td>
                        <td><c:forEach items="${sessionScope.projects}" var="pt" varStatus="suoyin2">
                            <c:if test="${pt.projectid ==i.projectid }">${pt.site}</c:if>
                        </c:forEach>
                        </td>
                        <td><c:forEach items="${sessionScope.projects}" var="pt" varStatus="suoyin2">
                            <c:if test="${pt.projectid ==i.projectid }">${pt.gbindustry}</c:if>
                            </c:forEach>
                        </td>
                        <td><c:forEach items="${sessionScope.projects}" var="pt" varStatus="suoyin2">
                            <c:if test="${pt.projectid ==i.projectid }">${pt.industry}</c:if>
                        </c:forEach>
                        </td>
                        <td><c:forEach items="${sessionScope.projects}" var="pt" varStatus="suoyin2">
                            <c:if test="${pt.projectid ==i.projectid }">${pt.investment}</c:if>
                        </c:forEach>
                        </td>
                        <td><fmt:formatDate value="${i.savetime}" pattern="yyyy-MM-dd"/></td>
                        <td>${i.fromsite}</td>
                        <td>${i.receivesite}</td>
                    </tr>
                    </tbody>
            ...
            <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/projectsend/guanli?pn=1">首页</a>

                <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/projectsend/guanli?pn=${pageInfo.pageNum-1}"
                ...
                <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                    ...
                    <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/projectsend/guanli?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                    </c:if>
                </c:forEach>
                ...
                <a class="paginate_button next disabled"
                   href="<%=request.getContextPath()%>/projectsend/guanli?pn=${pageInfo.pages}">末页</a>
            </div>
        </div>
    </div>
