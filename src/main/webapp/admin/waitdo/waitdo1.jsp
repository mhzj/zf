...
%>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 待办事项<span class="c-gray en">&gt;</span> <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <form id="myform" name="myform" method="post" >
        ...
        <span class="l">
            ...
            <span class="r">共有数据：<strong>${searchwaitdo.total}</strong> 条</span>
        </div>
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer">
                    ...
                    <c:forEach items="${searchwaitdo.list}" var="i" varStatus="suoyin1" >
                        <tbody>
                        <tr class="text-c">
                ...
                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                    当前第：${searchwaitdo.pageNum}页，总共：${searchwaitdo.pages}页，总共：${searchwaitdo.total}条记录 </div>

                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/waitdo/searchwaitdo?pn=1&content='<%=content%>'">首页</a>

                    <c:if test="${searchwaitdo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/waitdo/searchwaitdo?pn=${searchwaitdo.pageNum-1}&content='<%=content%>'"
                             aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
                </a></span>
                    </c:if>

                    <c:forEach items="${searchwaitdo.navigatepageNums }" var="page_Num">
                        <c:if test="${page_Num == searchwaitdo.pageNum }">
                        <span><a  class="paginate_button current" href="#">${ page_Num}</a>
                        </span>
                        </c:if>
                        <c:if test="${page_Num != searchwaitdo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/waitdo/searchwaitdo?pn=${ page_Num}&content='<%=content%>'">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    <c:if test="${searchwaitdo.hasNextPage }">
                    <span><a href="<%=request.getContextPath()%>/waitdo/searchwaitdo?pn=${searchwaitdo.pageNum+1}&content='<%=content%>'"
                             aria-label="Next"><span aria-hidden="true">&raquo;</span></a></span>
                    </c:if>
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/waitdo/searchwaitdo?pn=${searchwaitdo.pages}&content='<%=content%>">末页</a>
                </div>
            </div>
        </div>
    </form>
