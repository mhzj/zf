...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 投资计划 <span class="c-gray en">&gt;</span> 滚动计划管理 <span class="c-gray en">&gt;</span> 待报区 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
        ...
        <span class="l">
           <input type="button" class="btn btn-primary radius"  value="报送"  onclick="baosong()"  />
            ...
            <input type="button" class="btn btn-primary radius"  value="退回"  onclick="tuihui()"  />
             <input type="button" class="btn btn-primary radius" value="纳为本级滚动计划"  onclick="naru()"  />
        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/rollplan/daibao?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/rollplan/daibao?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/rollplan/daibao?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/rollplan/daibao?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <script type="text/javascript">
        ...
        function baosong( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("报送计划将同时被纳入本级滚动计划，您确定报送这" + count + "个项目吗?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/daibaobaosong?ids=" + result;
                }
            }
        }
        function tuihui( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定退回这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/daibaotuihui?ids=" + result;
                }
            }
        }
        ...
        function jiasuo() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("加锁后下级报送部门将无法对项目进行修改,您确定加锁这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/countydaibaojiasuo?ids=" + result;
                }
            }
        }
        function  jiesuo() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("解锁后将允许下级报送部门对项目进行修改,您确定解锁这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/countydaibaojiesuo?ids=" + result;
                }
            }
        ...
        function  naru() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("您确定将这" + count + "个项目纳入本级滚动计划不报送吗?")) {
                    window.location.href = "<%=request.getContextPath()%>/rollplan/countydaibaonaru?ids=" + result;
                }
            }
        }
        function update() {
