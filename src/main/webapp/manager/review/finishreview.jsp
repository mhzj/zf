...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 项目管理 <span class="c-gray en">&gt;</span> 项目审核入库 <span class="c-gray en">&gt;</span> 已审核 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div >
        <form action="<%=request.getContextPath()%>/review/search" method="post">
            项目名称:<input type="text" name="projectname"   style="width:170px" class="input-text">
            项目类型：<input type="text" name="projectype"   style="width:170px" class="input-text">
    ...
    <form id="myform" name="myform" method="post" action="">
        ...
        <span class="l">
            <input type="button" class="btn btn-primary radius" id="button" value="退回"  onclick="tuihui()" />
        </span>
        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/review/finishreview?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/review/finishreview?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/review/finishreview?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/review/finishreview?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
        ...
        function tuihui( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定退回这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/review/tuihui?ids=" + result;
                }
            }
        }
