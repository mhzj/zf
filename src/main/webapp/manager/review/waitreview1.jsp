...
<%
    ...
    String projectype=request.getParameter("projectype");
...
<div class="page-container">
    <div >
        <form action="<%=request.getContextPath()%>/review/search" method="post">
    ...
    <form id="myform" name="myform" method="post" action="">
        ...
        <span class="l">
            ...
            <span class="r">共有数据：<strong>${searchreview.total}</strong> 条</span>
        </div>
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer">
                    ...
                    <c:forEach items="${searchreview.list}" var="i" varStatus="suoyin1" >
                        <tbody>
                        <tr class="text-c">
                            <td><input name="bid${suoyin1.index+1}"  type="checkbox" value="${i.id }"></td>
                            <td><c:if test="${suoyin1.index==0}">${(searchreview.pageNum-1)*6+1}
                                <c:set var="startnum" value="${(searchreview.pageNum-1)*6+1}"/>
                            </c:if>
                ...
                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                    当前第：${searchreview.pageNum}页，总共：${searchreview.pages}页，总共：${searchreview.total}条记录 </div>

                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/review/search?pn=1&projectname='<%=projectname%>'&projectype='<%=projectype%>'&site='<%=site%>'">首页</a>

                    <c:if test="${searchreview.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/review/search?pn=${searchreview.pageNum-1}&projectname='<%=projectname%>'&projectype='<%=projectype%>'&site='<%=site%>'"
                             aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
                </a></span>
                    </c:if>

                    <c:forEach items="${searchreview.navigatepageNums }" var="page_Num">
                        <c:if test="${page_Num == searchreview.pageNum }">
                        <span><a  class="paginate_button current" href="#">${ page_Num}</a>
                        </span>
                        </c:if>
                        <c:if test="${page_Num != searchreview.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/review/search?pn=${ page_Num}&projectname='<%=projectname%>'&projectype='<%=projectype%>'&site='<%=site%>'">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    <c:if test="${searchreview.hasNextPage }">
                    <span><a href="<%=request.getContextPath()%>/review/search?pn=${searchreview.pageNum+1}&projectname='<%=projectname%>'&projectype='<%=projectype%>'&site='<%=site%>'"
                             aria-label="Next"><span aria-hidden="true">&raquo;</span></a></span>
                    </c:if>
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/review/search?pn=${searchreview.pages}&projectname='<%=projectname%>'&projectype='<%=projectype%>'&site='<%=site%>'">末页</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <script type="text/javascript">
        ...
        function insert() {
            window.location.href = "<%=request.getContextPath()%>/manager/review/shprojectadd.jsp";
        }
        function ruku( ) {
            var result = "";
            var count = 0;
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定入库这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/review/ruku?ids=" + result;
                }
            }
        }
        function update() {
        ...
        function delmore() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定删除这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/review/delmore?ids=" + result;
                }
            }
        }
    </script>
