...
%>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 意见反馈 <span class="c-gray en">&gt;</span> <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div >
        <form action="<%=request.getContextPath()%>/advise/countysearchadvise" method="post">
            意见内容:<input type="text" name="content" id=""  style="width:170px" class="input-text">
            状态：<input type="text" name="status" id=""  style="width:170px" class="input-text">
    ...
    <form id="myform" name="myform" method="post" action="">
        ...
        <span class="l">
            <a href="javascript:;" onclick="add()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 新增</a>

        </span>
            <span class="r">共有数据：<strong>${searchadvise.total}</strong> 条</span>
        </div>
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                <table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer">
                    ...
                    <c:forEach items="${searchadvise.list}" var="i" varStatus="suoyin1" >
                        <tbody>
                        <tr class="text-c">
                            <td><input name="bid${suoyin1.index+1}"  type="checkbox" value="${i.id}"></td>
                            <td>
                                <c:if test="${suoyin1.index==0}">${(searchadvise.pageNum-1)*6+1}
                                    <c:set var="startnum" value="${(searchadvise.pageNum-1)*6+1}"/>
                                </c:if>
                                <c:if test="${suoyin1.index!=0}">${startnum+suoyin1.index}
                                </c:if>
                            </td>
                            <td>${i.content}</td>
                            <td><fmt:formatDate value="${i.time}" pattern="yyyy-MM-dd"/>
                            <td>${i.telphone}</td>
                        </tr>
                        </tbody>
                    </c:forEach>
                ...
                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                    当前第：${searchadvise.pageNum}页，总共：${searchadvise.pages}页，总共：${searchadvise.total}条记录 </div>

                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/advise/countysearchadvise?pn=1&content='<%=content%>'&status='<%=status%>'">首页</a>

                    <c:if test="${searchadvise.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/advise/countysearchadvise?pn=${searchadvise.pageNum-1}&content='<%=content%>'&status='<%=status%>'"
                             aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
                </a></span>
                    </c:if>

                    <c:forEach items="${searchadvise.navigatepageNums }" var="page_Num">
                        <c:if test="${page_Num == searchadvise.pageNum }">
                        <span><a  class="paginate_button current" href="#">${ page_Num}</a>
                        </span>
                        </c:if>
                        <c:if test="${page_Num != searchadvise.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/advise/countysearchadvise?pn=${ page_Num}&date1='content='<%=content%>'&status='<%=status%>'">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    <c:if test="${searchadvise.hasNextPage }">
                    <span><a href="<%=request.getContextPath()%>/advise/countysearchadvise?pn=${searchadvise.pageNum+1}&content='<%=content%>'&status='<%=status%>'"
                             aria-label="Next"><span aria-hidden="true">&raquo;</span></a></span>
                    </c:if>
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/advise/countysearchadvise?pn=${searchadvise.pages}&content='<%=content%>'&status='<%=status%>'">末页</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <script type="text/javascript">
        ...
        function add() {
            window.location.href = "<%=basePath %>admin/advise/adviseadd.jsp";
        }
    </script>
</body>
</html>