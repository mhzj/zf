...
<div class="page-container">
        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/scheduling/citybianzhi?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/scheduling/citybianzhi?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/scheduling/citybianzhi?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/scheduling/citybianzhi?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <script type="text/javascript">
        ...
        function taskadd() {
            window.location.href = "<%=request.getContextPath()%>/city/tiaodu/insert.jsp";
        }
        function delmore( ) {
            var result = "";
            var count = 0;
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定删除这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/scheduling/citydelmore?ids=" + result;
                }
            }
        }
        function update() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定修改这个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/scheduling/cityupdate?ids=" + result;
                }
            }
        }
        function xiafa( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定下发这" + count + "个调度任务?")) {
                    window.location.href = "<%=request.getContextPath()%>/scheduling/cityxiafa?ids=" + result;
                }
            }
        }
    </script>
