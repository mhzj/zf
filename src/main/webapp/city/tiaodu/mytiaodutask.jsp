...
<div class="page-container">
    ...
    <form id="myform" name="myform" method="post" action="">
        ...
        <span class="l">
            <input type="button" class="btn btn-primary radius" id="button" value="批量确认"  onclick="queren()"  />
        </span>
        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                    ...
                    <c:forEach items="${pageInfo.list}" var="i" varStatus="suoyin1" >
                        ...
                        <tr class="text-c">
                            ...
                            <td><c:forEach items="${sessionScope.rollplans}" var="pt" varStatus="suoyin2">
                                <c:if test="${pt.projectid ==i.projectid }"><a href="<%=request.getContextPath()%>/scheduling/citytiaoduwrite?schedulingid=${i.schedulingid}&projectid=${i.projectid }&rollplanid=${pt.rollplanid}">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/scheduling/cityguanli?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/scheduling/cityguanli?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/scheduling/cityguanli?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/scheduling/cityguanli?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <script type="text/javascript">
        ...
        function queren( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("您确定将这" + count + "个调度项目状态更新为已确认吗？")) {
                    window.location.href = "<%=request.getContextPath()%>/scheduling/cityqueren?ids=" + result;
                }
            }
        }
    </script>
