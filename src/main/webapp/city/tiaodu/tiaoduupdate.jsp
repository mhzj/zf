...
<div class="page-container">
    <form action="${pageContext.request.contextPath }/scheduling/citysave?schedulingid=${sessionScope.updatescheduling.schedulingid}" method="post" class="form form-horizontal" id="form-article-add" >
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="${sessionScope.updatescheduling.name}" placeholder="" name="name">
            </div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
               <span class="select-box">
				<select name="stype" class="select">
					<option selected="selected" value="${sessionScope.updatescheduling.stype}">${sessionScope.updatescheduling.stype}</option>
                    <c:if test="${sessionScope.updateproject.stype!='三年滚动计划调度'}">
                        <option value="三年滚动计划调度">三年滚动计划调度</option></c:if>
                    <c:if test="${sessionScope.updateproject.stype!='年度计划调度'}">
                        <option value="年度计划调度">年度计划调度</option></c:if>
				</select>
				</span>
            </div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select name="cycle" class="select">
                    <option selected="selected" value="${sessionScope.updatescheduling.cycle}">${sessionScope.updatescheduling.cycle}</option>
                     <c:if test="${sessionScope.updateproject.cycle!='月报'}">
                         <option value="月报">月报</option></c:if>
                     <c:if test="${sessionScope.updateproject.cycle!='季报'}">
                         <option value="季报">季报</option></c:if>
                     <c:if test="${sessionScope.updateproject.cycle!='半年报'}">
                         <option value="半年报">半年报</option></c:if>
                     <c:if test="${sessionScope.updateproject.cycle!='年报'}">
                         <option value="年报">年报</option></c:if>
				</select>
				</span> </div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="${sessionScope.updatescheduling.tdstatus}" placeholder="" name="tdstatus" readonly="readonly">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">调度开始时间：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="tdstart" value="<fmt:formatDate value="${sessionScope.updatescheduling.tdstart}" pattern="yyyy-MM-dd"/>" class="input-text Wdate" style="width:180px;">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">调度截止时间：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'datemin\')}' })" id="datemax" name="tdend" value="<fmt:formatDate value="${sessionScope.updatescheduling.tdend}" pattern="yyyy-MM-dd"/>" class="input-text Wdate" style="width:180px;">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>调度要求：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <textarea name="requirement" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)">${sessionScope.updatescheduling.requirement}</textarea>
