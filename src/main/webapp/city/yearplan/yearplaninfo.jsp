...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 投资计划 <span class="c-gray en">&gt;</span> 年度计划管理 <span class="c-gray en">&gt;</span> 编制区 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <form id="myform" name="myform" method="post" action="">
        ...
        <span class="l">
            <input type="button" class="btn btn-primary radius" id="button" value="从滚动计划库挑选项目"  onclick="tiaoxuan()"  />
        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/rollplan/citybianzhiguanli?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/rollplan/citybianzhiguanli?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/rollplan/citybianzhiguanli?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/rollplan/citybianzhiguanli?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <script type="text/javascript">
        ...
        function tiaoxuan() {
            window.location.href = "<%=request.getContextPath()%>/yearplan/yearcityguanli";
        }
        ...
        function projectsend( ) {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定推送这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/yearplan/citysenddaibao?ids=" + result;
                }
            }
        }
        function delmore() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定删除这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/yearplan/citybianzhidelete?ids=" + result;
                }
            }
        }
    </script>
