...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 项目管理 <span class="c-gray en">&gt;</span> 本级项目库 <span class="c-gray en">&gt;</span> 政府投资项目库 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <form id="myform" name="myform" method="post" action="">
        ...
        <span class="l">
             ...
             <input type="button" class="btn btn-primary radius"  value="解锁"  onclick="jiesuo()"  />   </span>
        ...
        <div class="mt-20">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                    <a class="paginate_button previous disabled" href="<%=request.getContextPath()%>/projectlibrary/cityzftz?pn=1">首页</a>

                    <c:if test="${pageInfo.hasPreviousPage }">
                    <span><a href="<%=request.getContextPath()%>/projectlibrary/cityzftz?pn=${pageInfo.pageNum-1}"
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                        ...
                        <c:if test="${page_Num != pageInfo.pageNum }">
                        <span><a href="<%=request.getContextPath()%>/projectlibrary/cityzftz?pn=${ page_Num}">${
                                page_Num}</a>
                        </span>
                        </c:if>
                    </c:forEach>
                    ...
                    <a class="paginate_button next disabled"
                       href="<%=request.getContextPath()%>/projectlibrary/cityzftz?pn=${pageInfo.pages}">末页</a>
                </div>
            </div>
        </div>
    </form>
        ...
        function tuihui() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("确定将这" + count + "个项目取消储备?")) {
                    window.location.href = "<%=request.getContextPath()%>/projectlibrary/citytuihui?bz=1&ids=" + result;
                }
            }
        }
        function jiasuo() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("加锁后下级报送部门将无法对项目进行修改,您确定加锁这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/projectlibrary/cityjiasuo?bz=1&ids=" + result;
                }
            }
        }
        function  jiesuo() {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                if (window.confirm("解锁后将允许下级报送部门对项目进行修改,您确定解锁这" + count + "个项目?")) {
                    window.location.href = "<%=request.getContextPath()%>/projectlibrary/cityjiesuo?bz=1&ids=" + result;
                }
            }
        }
    </script>
